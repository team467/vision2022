...
public class FrameSource {
    ...
    public VideoFrame allocateVideoFrame(StreamProfile profile, Frame original, int bpp, int width, int height,
            int stride) {
        return allocateVideoFrame(profile, original, bpp, width, height, stride, Extension.VideoFrame);
    }

    public VideoFrame allocateVideoFrame(StreamProfile profile, Frame original, int bpp, int width, int height,
            int stride, Extension extension) {
        rs2_frame fref = rs2_allocate_synthetic_video_frame(instance, profile.getInstance(), original.getInstance(),
                bpp, width, height, stride, extension.getIndex(), RealSenseError.getInstance());
    ...
    public FrameList allocateCompositeFrame(Frame... frames) {
        ...
        rs2_frame frame_ref = rs2_allocate_composite_frame(instance, frameRefs[0], frameRefs.length,
